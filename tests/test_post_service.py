...
def test_add_a_paragraph_to_a_post(a_session, a_post, post_service):
    request = AddParagraphRequest(
        session_id=a_session.id,
        post_id=a_post.id,
        text="Some text")
    a_post_with_paragraph = post_service.add_paragraph_to_post(request)
    assert len(a_post_with_paragraph.contents) == 1
    assert post_service.find_post_by_id(a_post.id) == a_post_with_paragraph
